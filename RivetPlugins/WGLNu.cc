...
class WGLNu : public Analysis {
 public:
  ...
  RIVET_DEFAULT_ANALYSIS_CTOR(WGLNu);
  ...
  void init() {
    ...
    book(_h["baseline_photon_pt"], "p_pt", 40, 0, 1500);
    book(_h["baseline_phi"], "phi", 10, -PI, PI);
    book(_h["baseline_photon_eta"], "p_eta",20, -2.5, 2.5);
    book(_h["baseline_leppho_dr"], "leppho_dr", 20, 0.7, 4);
    book(_h["baseline_leppho_deta"], "leppho_deta", 40, -5, 5);
    book(_h["baseline_mt_cluster"], "mt_cluster", 25, 0, 500);
    book(_h["baseline_njet"], "njet", 7, 0, 7);
    book(_h["raz_leppho_deta"], "raz_leppho_deta", 40,  -5, 5);
    
    book(_h["eft_photon_pt_phi_0"], "pt_phi_0", 40, 0, 1500);
    book(_h["eft_photon_pt_phi_1"], "pt_phi_1", 40, 0, 1500);
    book(_h["eft_photon_pt_phi_2"], "pt_phi_2", 40, 0, 1500);
  }
  ...
  void analyze(const Event& event) {
    ...
    if (leptons.size() >= 1 && photons.size() >= 1 && neutrinos.size() >= 1) {
      ...
      if (vars_.l0_pt > lepton_pt_cut_ && std::abs(vars_.l0_eta) < lepton_abs_eta_cut_ &&
        ...
        _h["baseline_phi"]->fill(vars_.true_phi);
  ...
  void finalize() {
    ...
    for (std::string const& x :
         {"baseline_photon_pt", "baseline_photon_eta", "baseline_phi", "baseline_leppho_dr", "baseline_leppho_deta",
...
WGLNu::WGSystem::WGSystem(Particle const& lep, Particle const& neu,
